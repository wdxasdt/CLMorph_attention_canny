...
from slices_canny import slices_canny
...
class RegistrationModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.loss_names = ["total", "recon", "smooth", "contrastive","canny"]
    ...
    def set_input(self, input):
        def get_transform_train(img_size=(72, 96, 72)):
            transform_list = []
    ...
    def backward_Reg(self):
        ...
        # # 边缘轮廓提取
        # print('w',wrapped_image.shape)
        wrapped_canny = wrapped_image.detach().cpu()
        # print('warp',wrapped_canny.shape)
        wrapped_canny = slices_canny(wrapped_canny)
        # print('wrapped_canny',wrapped_canny.shape)
        # MICCAI 2018
        # 边缘损失计算
        self.loss_canny = self.criterionRecon(wrapped_canny, self.fixed_canny)
        ...
        # backward
        self.loss_total = self.loss_recon + self.loss_smooth + self.loss_contrastive + 0.2*self.loss_canny
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            print('Total_Mask_Dice={:0.40f}. '.format(np.nanmean(dsc)))
