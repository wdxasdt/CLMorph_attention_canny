...
class LPBA40(BaseDataset):
    def initialize(self, opt):
        ...
        self.canny_image_path = self.opt.cannyroot
        ...
        canny_path = []
        for files in os.listdir(self.canny_image_path):
            file_path = os.path.join(self.canny_image_path,files)
            canny_path.append(file_path)
        self.canny_path = canny_path
        ...
        self.canny_img = {x: self.readVol(x) for x in self.canny_path}
        self.canny_img_whiten = {k: self.whitening(v) for k, v in self.canny_img.items()}
    ...
    def __getitem__(self, index):
        ...
        if self.is_training:
            ...
            m1 = self.moving_path[img_index]
            m2 = self.readVol(m1)
            m3 = self.whitening(m2)
            moving_img = self.whitening(self.readVol(self.moving_path[img_index]))
            f1 = self.moving_path[img_index]
            f2 = self.moving_fixed[f1]
            fixed_img = self.fixed_img_whiten[self.moving_fixed[self.moving_path[img_index]]]

            canny_img = os.path.join(self.canny_image_path,os.path.basename(f2))

            # canny_img = self.whitening(self.readVol(canny_img))
            canny_img = self.canny_img_whiten[canny_img]
            ...
            moving_img_pytorch, fixed_img_pytorch,canny_img_pytorch,moving_atlas_pytorch, fixed_atlas_pytorch = self.transform_train(
                [moving_img, fixed_img,canny_img,moving_atlas, fixed_atlas])
            ...
            return {'A': moving_img_pytorch,
                    ...
                    'C': fixed_img_pytorch,
                    'C_paths': os.path.join(self.canny_image_path,os.path.basename(f2))
                    }
        else:
            img_index = int(index % len(self.moving_path))
            ...
            return {'A': moving_img_pytorch,
                    ...
                    'B_paths': self.moving_fixed[self.moving_path[img_index]],
                    'C': fixed_img_pytorch,
                    'C_paths': os.path.join(self.canny_image_path, os.path.basename(self.moving_path[img_index]))
                    }
